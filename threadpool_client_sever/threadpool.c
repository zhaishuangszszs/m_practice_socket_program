...
ThreadPool *threadPoolCreate(int min, int max, int queueSize)
{
    ...
    do
    {
        ...
        pool->shutdown = 0;
         ...
         printf("init queueSize is %d\n",pool->queueSize);

        // 创建线程
        pthread_create(&pool->managerID, NULL, manager, pool);
        printf("create init manage thread %ld\n",pool->managerID);
        printf("创建 manger 后queueSize is %d\n",pool->queueSize);
        for (int i = 0; i < min; ++i)//liveNum的线程
        {
            pthread_create(&pool->threadIDs[i], NULL, worker, pool);
            printf("create init thread %ld\n",pool->threadIDs[i]);
        }
         printf("创建 worker 后queueSize is %d\n",pool->queueSize);
        return pool;
    } while (0);
...
void *worker(void *arg)
{
    ...
    while (1)
    {
        ...
        free(task.arg);//arg参数一般在外面malloc申请需要线程池来释放
        task.arg = NULL;
...
void *manager(void *arg)
{
    ...
    while (!pool->shutdown)//当线程池不关闭时
    {
        ...
        // 添加线程
        // 任务的个数++工作中的>存活的线程个数（工作中的queuesize会减一加工作中的才是真正的queueSzie） && 存活的线程数<最大线程数
...
void threadPoolAdd(ThreadPool *pool, void (*func)(void *), void *arg)
{
    ...
    pool->queueSize++;
    printf("threadPoolAdd queueSize is %d\n",pool->queueSize);
